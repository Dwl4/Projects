...
export default function SignupPage() {
  ...
  const handleSubmit = (e) => {
    ...
    // 필수 입력 필드 확인
    if (!formData.name || !formData.email || !formData.password || !formData.confirmPassword || !formData.nickname || !formData.address) {
      alert('요구사항을 입력해주세요.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center">
      ...
      <div className="w-[1600px] mx-auto flex flex-col items-center">
        ...
        {/* 회원가입 폼 */}
        <div className="w-[600px] h-[1050px] bg-white border border-[#bcbcbc] rounded-[30px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] p-[50px] flex flex-col items-center">
          {/* 제목 */}
          ...
          <form onSubmit={handleSubmit} className="w-[400px] flex flex-col gap-[20px]">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full h-[35px] px-[12px] border rounded-[10px] text-[11px] placeholder-[#acacac] ${
                  passwordError ? 'border-red-500' : passwordMatch ? 'border-green-500' : 'border-[#929292]'
                }`}
              />
              {passwordError && (
                <p className="text-red-500 text-[10px] mt-1 ml-1">{passwordError}</p>
              )}
              {passwordMatch && !passwordError && (
                <p className="text-green-500 text-[10px] mt-1 ml-1">{passwordMatch}</p>
              )}
            </div>
